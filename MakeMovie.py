...
def create_data_arrays(ms,mixangle,rates=False):
    ...
    if rates:
        ...
        n2p_st = np.zeros(Num_steps)
        p2n = np.zeros(Num_steps)
        p2n_st = np.zeros(Num_steps)
        Hub = np.zeros(Num_steps)
        
        eq_a_array = np.load('..../eq_arrs/eq_arrs/Luke-a.npy')
        eq_T_array = np.load('..../eq_arrs/eq_arrs/Luke-T.npy')
        eq_eta_array = np.load('..../eq_arrs/eq_arrs/Luke-eta.npy')
        ...
        for i in range(Num_steps):
            ...
            n2p_st[i] = nprate.varlam_np(a_tot[i], T_tot[i], f_nu(e_tot[i][:len_e]/a_tot[i],eq_Tcm[i]), e_tot[i][:len_e], eq_eta[i])
            p2n[i] = nprate.varlam_pn(a_tot[i], T_tot[i], y_tot[i][:len_e], e_tot[i][:len_e], eq_eta[i])
            p2n_st[i] = nprate.varlam_pn(a_tot[i], T_tot[i], f_nu(e_tot[i][:len_e]/a_tot[i],eq_Tcm[i]), e_tot[i][:len_e], eq_eta[i])
        ...
        for i in range(len(files)):
            ...
            k += 1
        Hub[0] = Hub[1] 
        np.savez(folder+'/movie-arrays',a=a_tot, y=y_tot, e=e_tot, T=T_tot, t=t_tot, ym=y_mat, d=d_tot, c=c_tot, np=n2p, np_SM=n2p_st, pn=p2n, pn_SM=p2n_st, Hub=Hub)
        return a_tot, y_tot, e_tot, T_tot, t_tot, y_mat, d_tot, c_tot, n2p, n2p_st, p2n, p2n_st, Hub
    else:
...
def create_movie_arrays(mH,mixangle,rates=False):
    #folder = "..../{}-{:.4}-FullTestNew/{}-{:.4}-FullTestNew".format(mH,mixangle,mH,mixangle)
    
    data = np.load("..../{}-{:.4}-FullTestNew/{}-{:.4}-FullTestNew/mass_{}_mix_{:.4}.npz".format(mH,mixangle), allow_pickle=True)
    ...
    for i in range(Num_steps):
        #len_e = len(np.where(e_tot[i] > 0)[0])+1
        n2p[i] = nprate.varlam_np(a_tot[i], T_tot[i], fe_tot[i], e_tot[i], eq_eta[i])
        p2n[i] = nprate.varlam_pn(a_tot[i], T_tot[i], fe_tot[i], e_tot[i], eq_eta[i])
    ...
    for i in range(1,len(a_tot)):
        dtda = mv.time_derivative(a_tot[i],T_tot[i])
        Hub[i] = (dtda * a_tot[i])**(-1)
    ...
    np.savez(folder+'/movie-arrays', a=a_tot, fe=fe_tot, e=e_tot, T=T_tot, t=t_tot, np=n2p, np_SM=n2p_st, pn=p2n, pn_SM=p2n_st, Hub=Hub, Hub_SM=Hub_st)
    return a_tot, fe_tot, e_tot, T_tot, t_tot, n2p, n2p_st, p2n, p2n_st, Hub, Hub_st





def make_only_0and3_video(mH,mixangle):
    fn = "..../{}-{:.4}-FullTestNew/{}-{:.4}-FullTestNew".format(mH,mixangle)
    ...
    if not os.path.isfile(fn+'/movie-arrays.npz'):
        ...
        create_movie_arrays(mH,mixangle,True)
    arrs = np.load(fn+'/movie-arrays.npz', allow_pickle=True)
    ...
    def init3():
        ...
        np_plot.set_data([],[])
        np_SM_plot.set_data([],[])
        return pn_plot, pn_SM_plot, np_plot, np_SM_plot
    ...
    def animate0(i):
        ...
        nu.set_data(eps, eps**3 * fev / (2 * np.pi**3))
    ...
    def animate3(i):
        ...
        pn_SM_plot.set_data(eq_T_array[:(i+1)], p2n_SM)
        np_plot.set_data(T_vec, n2p)
        np_SM_plot.set_data(eq_T_array[:(i+1)], n2p_SM)
    ...
    pn_SM_plot, = ax3.loglog([],[],color='rebeccapurple',linewidth=1,linestyle='--')
    np_plot, = ax3.loglog([],[],color='salmon',linewidth=2,label=r"$\lambda_{n \rightarrow p}$")
    np_SM_plot, = ax3.loglog([],[],color='salmon',linewidth=1,linestyle='--')  
...
def make_video(mH,mixangle):
    ...
    def animate3(i):
        ...
        Hub = arrs['Hub'][:(i+1)]
        n2p = arrs['np'][:(i+1)] / Hub
        n2p_SM = arrs['np_SM'][:(i+1)] / Hub
        p2n = arrs['pn'][:(i+1)] / Hub
        p2n_SM = arrs['pn_SM'][:(i+1)] / Hub
        ...
        return pn_plot, pn_SM_plot, np_plot, np_SM_plot, time_text
